...
namespace Plan.Plandokument
{
	public class UtilityLog : Utility
	{
        ...
        public static async Task LogAsync(string logMessage, LogLevel logLevel)
        {
            await LoggerAsync(logMessage, DateTime.Now, logLevel);
        }
        ...
        public static async Task LogAsync(string logMessage, DateTime dateTimeStamp, LogLevel logLevel)
        {
            await LoggerAsync(logMessage, dateTimeStamp, logLevel);
        }
        ...
        private static async Task LoggerAsync(string logMessage, DateTime dateTimeStamp, LogLevel logLevel)
        {
            logDirectoryExist();
            ...
            {
                // skriv till basfilen
                await WriteLoggTextToFileAsync(logMessage, dateTimeStamp, logLevel, logFile);
            }
            else
            {
                //dela upp basfilen men ej till fler delfiler än inställning medger
        ...
        /// <summary>
        /// Skriver meddelandet asynkront till logg-/textfil enligt visst format under förutsättning att filen existerar.
        /// </summary>
        /// <param name="logMessage">Meddelande som ska loggas.</param>
        /// <param name="dateTimeStamp">Datum och tid för meddelandet.</param>
        /// <param name="logFile">Fil som logg skrivs till.</param>
        private static async Task WriteLoggTextToFileAsync(string logMessage, DateTime dateTimeStamp, LogLevel logLevel, string logFile)
        {
            // Open the log file for append and write the log
            using (StreamWriter sw = new StreamWriter(logFile, true))
            {
                string _dateTimeStamp = dateTimeStamp.ToString("yyyy-MM-dd HH:mm:ss");
                //sw.WriteLine($"{_dateTimeStamp}|\t{logLevel.ToString()}|\t{logMessage}");
                await sw.WriteLineAsync(string.Format("{0,20} {1,7} {2}",
                     _dateTimeStamp + "|",
                     logLevel.ToString() + "|",
                     logMessage
                    ));
